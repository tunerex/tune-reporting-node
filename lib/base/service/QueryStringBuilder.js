...
/**
 * Incrementally builds query string for Tune Management API action.
 *
...
QueryStringBuilder.prototype.encode = function (name, value) {
  this.query_string_dict[name] = value;
};
...
QueryStringBuilder.prototype.toString = function () {
  ...
  for (name in this.query_string_dict) {
    if (this.query_string_dict.hasOwnProperty(name)) {
      value = this.query_string_dict[name];
