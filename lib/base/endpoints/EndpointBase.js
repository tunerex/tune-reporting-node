...
function EndpointBase(
  ...
  if (validate_fields
      && !_.isBoolean(validate_fields)
      ) {
    throw new InvalidArgument('Parameter "validate_fields" is not valid.');
  }

  this.controller = controller;
  this.api_key = api_key;
  this.validate_fields = validate_fields || false;
 ...
 * @method define
 *
 * @return {EventEmitter} Event response
 */
EndpointBase.prototype.define = function () {
  return this.request('define');
};
 ...
 * @method getFields
 *
 * @param integer enum_fields_selection   What subset of fields for this endpoint
 *                                        to return.
 *
  ...
  try {
    ...
    async.series({
      ...
      filter_fields: function (next) {
        if ((enum_fields_selection & EndpointBase.TUNE_FIELDS_ALL) ||
            (!(enum_fields_selection & EndpointBase.TUNE_FIELDS_DEFAULT)
            && (enum_fields_selection & EndpointBase.TUNE_FIELDS_RELATED))
            ) {
        ...
        if (enum_fields_selection & EndpointBase.TUNE_FIELDS_RECOMMENDED) {
        ...
        _.each(self.fields, function (field_info, field_name) {
          if (((enum_fields_selection & EndpointBase.TUNE_FIELDS_ENDPOINT)
               || !(enum_fields_selection & EndpointBase.TUNE_FIELDS_DEFAULT))
              && !field_info.related
              ) {
          ...
          if (!(enum_fields_selection & EndpointBase.TUNE_FIELDS_RELATED)
              && !(enum_fields_selection & EndpointBase.TUNE_FIELDS_MINIMAL)
              && field_info.related
              ) {
            return;
          }

          if ((enum_fields_selection & EndpointBase.TUNE_FIELDS_DEFAULT)
              && field_info['default']
              ) {
            fields_filtered[field_name] = field_info;
            return;
...
EndpointBase.prototype._requestEndpointFields = function (callback) {
  ...
  client_request.on('success', function (client_response) {
    var
      data = client_response.getData(),
      http_code = client_response.getHttpCode(),
      request_url = client_response.getRequestUrl(),
  ...
  client_request.on('error', function (client_response) {
    var
      data = client_response.getData(),
      http_code = client_response.getHttpCode(),
      request_url = client_response.getRequestUrl(),
...
EndpointBase.prototype.validateFilter = function (filter) {
  ...
  var
    ...
    filter_quoted_value_regex = /^\'[\w\%\$\@\.\-\_]+\'$/gi,
    filter_number_value_regex = /^[\d\.]+$/gi,
    filter_field_regex = /^[a-z0-9\.\_]+$/gi;
 ...
 */
EndpointBase.prototype.validateDateTime = function (param_name, date_time) {
  if (!param_name || !_.isString(param_name) || (0 === param_name.length)) {
    throw new InvalidArgument(
      'param_name'
    );
  }
  if (!date_time || !_.isString(date_time) || (0 === date_time.length)) {
    throw new InvalidArgument(
      param_name
    );
  }
  ...
  if (regex_date.test(date_time)) {
    return date_time;
  }

  if (regex_date_time.test(date_time)) {
    return date_time;
  }

  throw new TuneSdkError(
    util.format('Invalid parameter "%s" provided: "%s"', param_name, date_time)
  );
};
...
EndpointBase.prototype._statusReport = function (
  ...
  if (!export_controller || !_.isString(export_controller) || (0 === export_controller.length)) {
    throw new InvalidArgument(
      'export_controller'
    );
  }
  if (!export_action || !_.isString(export_action) || (0 === export_action.length)) {
    throw new InvalidArgument(
      'export_action'
    );
  }
  if (!job_id || !_.isString(job_id) || (0 === job_id.length)) {
    throw new InvalidArgument(
      'job_id'
    );
  }
...
EndpointBase.prototype._fetchReport = function (
  ...
  if (!this.api_key
      || !_.isString(this.api_key)
      || (0 === this.api_key.length)
      ) {
    throw new InvalidArgument(
      'api_key'
    );
  ...
  // Start the request
    ...
    async.until(
      ...
      function (next) {
        ...
        client_request.once('success', function (response) {
          status_response = response;

          var
            request_url = status_response.getRequestUrl(),
            response_http_code = status_response.getHttpCode(),
            response_errors = status_response.getErrors(),
          ...
          percent_complete = response_data.percent_complete;
        ...
        client_request.once('error', function (status_response) {
          next(status_response);
        });
      },
      function (err) {
